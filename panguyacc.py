...
## The following are the production rule for the pangu grammar
...
def Parser(file):
    ...
    print(y_data)
    print("-------------------------------")
    pangupar = yacc.yacc() # build the parser
    pangupar.parse(y_data,pangulex.lexer,False,False,None)
    print("parsing successful\n\n")
    if y_build_ptree:
        if y_show_ptree: # show the parsed tree
            print("parse tree:\n\n")
            y_ptree.showtree()
            print('-----------------------------')
        if y_deparse_ptree: # deparsed the AST
            print("deparse:\n\n")
            y_ptree.deparse()
            print('-----------------------------')
        if y_build_symbol_table: # build the symbol table
            symbol_table = seman(y_ptree,None)
            if y_show_symbol_table: # show the symbol table
...
# Build the parser with the given input file (pangu source code)
Parser(sys.argv[1])
